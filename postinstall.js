...
if (values.every((val) => typeof val === 'undefined')) {
  console.info(
    '[local-geocoder] post-install: No env variables detected. Doing nothing.'
  );
  process.exit(0);
}
...
try {
  console.info('[local-geocoder] post-install: Starting.');
  geocoder.init(
    {
    ...
    function () {
      console.info('[local-geocoder] post-install: Finished.');
      process.exit(0);
    }
  );
} catch (error) {
  if (GEOCODER_POSTINSTALL_FAIL_SILENTLY?.toLowerCase() === 'true') {
    console.warn(
      '[local-geocoder] post-install: An error occurred. Detected fail-silently flag.',
      error
    );
    process.exit(0);
  } else {
    console.error(
      '[local-geocoder] post-install: An error occurred.',
      error
    );
    process.exit(1);
  }
