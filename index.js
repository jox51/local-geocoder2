...
var geocoder = {
    ...
    if (options.countries.length === 0) {
      ...
      async.parallel(
        ...
        function (err, countriesData) {
          ...
          debug(
            'Started building geonames for specific countries (this may take a while)'
          );
          if (that._geonames) {
  ...
  _lookup: function (terms, maxResults, callback) {
    ...
    console.log('Lookup input:', terms);
    ...
    var functions = [];
    functions[0] = function(innerCallback) {
      // Extract search parameters
      const searchName = String(terms.name || '');
      const searchState = terms.admin1Code ? String(terms.admin1Code) : null;
      const searchCountry = terms.countryCode ? String(terms.countryCode) : null;
      ...
      // Validate we have a name to search for
      if (!searchName) {
      ...
      // Filter geonames based on criteria
      const result = that._geonames.filter((geoname) => {
        if (!geoname || typeof geoname.name !== 'string') return false;

        // Check main name and alternate names
        const nameMatch = normalizeString(geoname.name) === searchNameNormalized;
        const alternateMatch = !nameMatch && geoname.alternateNames && 
          geoname.alternateNames.split(',').some(alt => 
            normalizeString(alt) === searchNameNormalized
          );

        // If searching for a 2-letter code that could be state or country,
        // match either one when the other isn't specified
        const stateMatch = !searchState || 
          geoname.admin1Code === searchState ||
          (!searchCountry && searchState.length === 2 && geoname.countryCode === searchState);
        
        const countryMatch = !searchCountry || 
          geoname.countryCode === searchCountry ||
          (!searchState && searchCountry.length === 2 && geoname.admin1Code === searchCountry);

        return (nameMatch || alternateMatch) && (stateMatch || countryMatch);
      });
      ...
      const sortedResults = result
        .sort((a, b) => {
          ...
          const aExact = normalizeString(a.name) === searchNameNormalized ? 1 : 0;
          const bExact = normalizeString(b.name) === searchNameNormalized ? 1 : 0;
      ...
      return innerCallback(null, sortedResults);
    };
